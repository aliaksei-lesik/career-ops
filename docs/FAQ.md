...
Windows does not create symlinks by default, so Git checks out the CLI skill entrypoints (`.claude/skills/`, `.opencode/skills/`, etc.) as plain pointer files instead of real symlinks. The installer and updater both detect this automatically: self-update is disabled in this fork, so materialize them by hand — copy `.agents/skills/career-ops/SKILL.md` over each pointer file, or re-clone on a machine where Git can create symlinks. No manual `mklink` or Developer Mode changes are needed.
