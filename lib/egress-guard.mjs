...
// Политика читается из checkout'а, а не из getCareerOpsRoot(). Корень данных
// задаётся переменной CAREER_OPS_ROOT, то есть выбирается тем же, кто запускает
// скрипт: разрешающий llm-endpoints.json в собственном каталоге снимал бы
// границу целиком. Решение о том, кому отдавать резюме сотрудников, принимает
// компания, поэтому файл лежит под git и проходит ревью, как код. Побочно это
// избавляет от копирования политики в каждый корень кандидата.
const CONFIG_PATH = fileURLToPath(new URL(`../${CONFIG_NAME}`, import.meta.url));
...
export function loadEgressAllowlist() {
  if (!existsSync(CONFIG_PATH)) return { allow: [], source: `${CONFIG_NAME} (отсутствует)` };
  try {
